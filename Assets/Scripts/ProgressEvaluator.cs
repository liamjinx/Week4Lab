...
public class ProgressEvaluator : MonoBehaviour
{
    ...
    [SerializeField] private uint studentNumber;
    ...
    private void Pass60Band()
    {
        ...
        Debug.LogWarning("PROGRESS EVALUATOR: Git repository not being automatically checked from Pass50 onwards. " +
            "Make sure you are adding files, committing changes, and pushing commits on your repository.");
#elif (!Pass60)
    ...
    private void Credit70Band()
    {
        ...
        if (laContents.Length == 0)
            throw new EvalFailedException("Can't find the LoadAssets.cs script in your Assets folder. " +
                "You should not be seeing this error, let the subject co-ordinator know.");
    ...
    private IEnumerator Cred70Coroutine()
    {
        ...
        if (!VectorApprox(redObj.transform.position, new Vector3(2.2f, 0, 0)))
            throw new EvalFailedException("The RedPrefab instance has not been instantiated to the correct location");
        ...
        if (!VectorApprox(blueObj.transform.position, new Vector3(-2.2f, 0, 0)))
            throw new EvalFailedException("The BluePrefab instance has not been instantiated to the correct location");
        ...
        // Eval Finished
        EvalPassMessage(nameof(GradeBand.Credit70));
    ...
    private IEnumerator HD90Coroutine()
    {
        ...
        // Eval Finished
        EvalPassMessage(nameof(GradeBand.HD90));
    ...
    private void HighDist100Band()
    {
        ...
        if (laContents.Length == 0)
            throw new EvalFailedException("Can't find the PrintAndHides.cs script in your Assets folder. " +
                "You should not be seeing this error, let subject co-ordinator know.");
        string laContent = File.ReadAllText(laContents[0]);
    ...
    private IEnumerator HD100Coroutine(GameObject redObj)
    {
        ...
        if (blueObj.GetComponent<Renderer>().enabled)
            throw new EvalFailedException("The blue object's renderer is enabled, it should be disabled by this time");

        // Eval Finished
        EvalPassMessage(nameof(GradeBand.HD100));
    ...
    private GradeBand EvalReader()
    {
        try
        {
            using BinaryReader reader = new BinaryReader(File.Open("ProjectSettings\\ProgEv", FileMode.Open));
    ...
    private static bool VectorApprox(Vector3 a, Vector3 b)
    {
